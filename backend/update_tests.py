...
for test_name, expected_values in test_results.items():
    ...
    if test_match:
        ...
        assertion_matches = re.finditer(r'\s+assert result\["([^"]+)"\] == Decimal\([^)]+\)', test_code)
        ...
        for match in assertion_matches:
            ...
            if key in expected_values:
                ...
                updated_assertion = f'    assert result["{key}"] == Decimal({expected_values[key]})\n'
                
                # Replace the assertion
                updated_test_code = updated_test_code.replace(full_match, updated_assertion)
